...
// ─── Connector SVG ─────────────────────────────────────────────────────────────
const ConnectorSVG = ({ ri, matches, rounds, totalH, numMap, gridSlots }: {
  ri: number; matches: BracketMatch[]; rounds: Round[]; totalH: number;
  numMap: Record<string, number>; gridSlots: ReadonlyMap<string, number>;
}) => {
  const W = COL_GAP;
  const paths: string[] = [];
  ...
  matches.forEach((m) => {
    const slot = gridSlots.get(String(m.id)) ?? 0;
    const parentSlot = Math.floor(slot / 2);
    const parentMatch = nextRoundMatches.find((candidate) => gridSlots.get(String(candidate.id)) === parentSlot);
...
export const SingleElimination = ({ matches }: { matches: BracketMatch[] }) => {
  ...
  const gridSlots = new Map<string, number>();
  const lastRound = rounds[rounds.length - 1];
  lastRound.matches.forEach((match, index) => gridSlots.set(String(match.id), index));

  for (let ri = rounds.length - 1; ri > 0; ri--) {
    const curr = rounds[ri];
    const prev = rounds[ri - 1];
    curr.matches.forEach(m => {
      const pSlot = gridSlots.get(String(m.id)) ?? 0;
      ...
      children.forEach(c => {
        ...
        gridSlots.set(String(c.id), pSlot * 2 + (feedsP0 ? 0 : 1));
      });
    });
  }
            ...
            <React.Fragment key={round.title}>
              <div style={{ position: "relative", width: CARD_W, flexShrink: 0,
                ...
                {round.matches.map((match) => {
                  const slot = gridSlots.get(String(match.id)) ?? 0;
              ...
              {ri < rounds.length - 1 ? (
                <ConnectorSVG ri={ri} matches={round.matches} rounds={rounds} totalH={totalH} numMap={numMap} gridSlots={gridSlots} />
              ) : null}
            </React.Fragment>
          ))}
