...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!token) return NextResponse.json({ message: "Log in to leave this tournament." }, { status: 401 });
  const { id } = await context.params;
  if (!/^[A-Za-z0-9_-]{1,128}$/.test(id)) return NextResponse.json({ message: "Invalid tournament identifier." }, { status: 400 });
  try {
    const response = await upstreamFetch("/v1/tournament/leave", { method: "POST", token, body: JSON.stringify({ criteria: { _id: id } }), allowErrorResponse: true });
    return NextResponse.json(await readResponseBody(response), { status: response.status });
  } catch (error) { return errorResponse(error, "Could not leave tournament."); }
}
